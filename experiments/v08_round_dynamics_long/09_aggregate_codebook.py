...
요약을 만든다. 출력은 ``outputs/v08_round_dynamics_long/codebook_lift_summary.json``.
...
def _discover_cells(seeds: list[int]) -> list[str]:
    """Scan ``outputs/v08_round_dynamics_long/seed{S}/*`` for cell directories
    ...
    found: set[str] = set()
    for seed in seeds:
        seed_dir = OUTPUT_DIR / "v08_round_dynamics_long" / f"seed{seed}"
...
def _load_cell_seed(seed: int, cell: str) -> dict:
    p = OUTPUT_DIR / "v08_round_dynamics_long" / f"seed{seed}" / cell / "codebook_lift.json"
    if not p.exists():
...
def main() -> None:
    ...
    ap.add_argument("--cells", type=str, nargs="*", default=None,
                    help="Override cell list. Default: auto-discover under "
                         "outputs/v08_round_dynamics_long/seed*/.")
    args = ap.parse_args()

    out_root = OUTPUT_DIR / "v08_round_dynamics_long"
