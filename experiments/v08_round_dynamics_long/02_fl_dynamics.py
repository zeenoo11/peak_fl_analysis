...
- batch=512, rounds=150, local_epochs=5, full participation (C=1.0).
  (v06 의 rounds=20, local_epochs=40 cell이 round 2~3에서 plateau에 도달하는 문제를
   FL 표준 (E, R) 비율 ~10 영역으로 옮긴 v08 long-round 변형.)
- Algorithm-specific extras: FedProx mu=0.01, FedRep head_epochs=1,
  Ditto lam=0.1, FedProto K=32 lambda_proto=0.1.

Output
------
``outputs/v08_round_dynamics_long/seed{S}/V6-Dyn-B-{Algo}/``
    ├── round_log.jsonl
    ├── final_state_dict.pt
...
def main() -> None:
    ...
    # Ditto returns both: final_state_dict = mean_personal (cold convention),
    # global_state_dict = FedAvg global trunk. Save the latter separately so
    # Phase 2 codebook stacking can use the global trunk for apples-to-apples
    # comparison with other algorithms (audit C1 from v08 critical review).
    if "global_state_dict" in result and result["global_state_dict"] is not None:
        torch.save(result["global_state_dict"], out_dir / "global_state_dict.pt")
