"""Re-run NHITS with the fixed MLP (PR #1 Copilot review fix).

Why
---
PR #1's Copilot review caught a bug in ``src/models/nhits.py``: the MLP
construction was adding the activation/dropout AFTER the last hidden
``Linear`` and missing them after the very first ``Linear``, producing
two consecutive activations between the last hidden Linear and the
``theta`` head. The fix matches NeuralForecast's official layout and
was committed in ``4fdab45``. Param count is unchanged (1,057,098), so
the architecture *shape* matches, but the activation pattern differs
and the existing v04 NHITS numbers (52.99 ± 1.64 PAPE) were obtained
with the buggy MLP — they are stale and must be re-run.

What this script does
---------------------
This is a thin wrapper around the parent folder's ``02_nf_train.py``
that locks ``--model nhits`` and writes to a different output directory
(``nf_nhits_fixed`` instead of ``nf_nhits``) so the old NHITS numbers
under ``seed{S}/nf_nhits/`` are preserved as historical record. We
duplicate (rather than import-from) the logic in ``02_nf_train.py``
because (a) ``02_nf_train.py`` writes hard-coded paths under
``seed{S}/nf_{model}/`` which we deliberately want to override, and
(b) the duplication keeps ``09_fix_rerun/`` self-contained. The
training protocol is otherwise bit-identical to the parent script:
centralised pooled training across the 80 train apts with per-apt
z-norm + stride=1 + plain MAE loss + bf16 autocast + batch=512.

CLI
---
    uv run python experiments/v04_full_baseline_comparison/09_fix_rerun/01_rerun_nhits.py --seed 42

Output
------
    outputs/v04_full_baseline_comparison/09_fix_rerun/seed{S}/nf_nhits_fixed/
        result.json   (same shape as 02_nf_train.py's NHITS row)
        best.pt       (best-val-MAE checkpoint)

Wall-clock: ~6 min per seed (matches parent ``02_nf_train.py`` for nhits).
"""
...
def build_pooled_loaders(train_apts: list[str], batch: int):
    """Pooled training across 80 train apts (per-apt z-norm + stride=1)."""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="v04 09_fix_rerun: re-run NHITS with fixed MLP.")
    ...
    out_dir = V04_FIX_OUT_ROOT / f"seed{args.seed}" / "nf_nhits_fixed"
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"[v04 fix-NHITS] seed={args.seed}  epochs={args.epochs}  batch={args.batch_size}  amp={use_amp}")
    gpu_start = _gpu_snapshot()
    print(f"[v04 fix-NHITS] GPU @start: {gpu_start}")
    ...
    print(f"[v04 fix-NHITS] {len(present)} apts, {n_train_windows} train windows")

    model = NHITS().to(DEVICE)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"[v04 fix-NHITS] params: {n_params}")
    ...
    print(f"[v04 fix-NHITS] GPU @end: {gpu_end}")
    print(f"[v04 fix-NHITS] cold: PAPE={cold_metrics['pape']:.2f}  HR@1={cold_metrics['hr@1']:.1f}  HR@2={cold_metrics['hr@2']:.1f}")
    print(f"[v04 fix-NHITS] train elapsed: {train_elapsed:.0f}s ({train_elapsed/60:.1f} min)")

    torch.save(best_state, out_dir / "best.pt")
    with open(out_dir / "result.json", "w") as fh:
        json.dump({
            "algorithm": "nf_nhits_fixed",
            "model": "nhits",
            "n_params": n_params,
            "seed": int(args.seed),
            ...
            "gpu_at_end": gpu_end,
            "comment": (
                "Re-run of v04 NHITS with the fixed MLP (PR #1 Copilot fix in commit "
                "4fdab45). The previous nf_nhits/ directory under seed{S}/ holds the "
                "stale (buggy-MLP) numbers; this directory holds the corrected ones. "
                "Param count unchanged (1,057,098); only the activation pattern between "
                "hidden Linears and the theta head changed."
            ),
        }, fh, indent=2)
    print(f"[v04 fix-NHITS] saved -> {out_dir}")
...
#   - 30 epochs (or earlier early-stop) of pooled training, ~6 min wall-clock.
#   - Cold PAPE in the same ballpark as the parent nf_nhits (~52 kW); the
#     fix is small enough that we expect <1 kW shift, but the exact number
#     can move either direction since two-consecutive-activations was a
#     genuine architectural error.
