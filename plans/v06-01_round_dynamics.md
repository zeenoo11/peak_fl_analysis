...
> validation* 으로 교체**한다. v01–v05의 cold 구조를 따르지 않으며, 100가구
...
- Hyperparameter: rounds = 20, local_epochs = 2, batch = 512, lr = 1e-3,
  wd = 1e-5, λ = 0.3, hr_weight = 0.1 (conference Phase A 기본값과
  bit-equivalent).
...
**G1.** **Centralised pooled SGD upper bound.** NBEATSxAux 를 100가구
...
FedProx, FedRep, Ditto, FedProto 각각을 100가구 위에서 conference hyperparameter
(rounds=20, local_epochs=2, batch=512) 로 학습. 라운드별 across-client val
...
3종 NF 를 같은 100가구 pooled training 위에서 한 번 학습하고 terminal val/test
...
100가구 val/test 위에서 한 번 zero-shot inference. trajectory 없음. **Gate
5 cell skip 가능** — conference 결과를 reuse 해도 평가 데이터가 cold 20가구
→ 100가구 internal val 로 바뀌었으므로 재측정 필요.

**G6.** **(옵션) FedSGD limit reference.** docs 분석의 두 representative
methodology 중 다른 한 쪽. 100가구 위에서 1 SGD step / round, R ≈ 60–80
...
- **80:20 split YAML / v10 households YAML reuse**. 사용 안 함; 100가구
  전부 internal split.
- **E (local_epochs) sweep**. conference 의 `local_epochs=2` 를 그대로
  사용. v06 의 비교축은 *알고리즘 종류* (FL 5종 + Proposed) 이지 E 가
  아니다.
...
- 모든 valid UMass 2016 apartments: `dataloader.umass.list_available_apartments
  ('2016')` → `filter_valid_apartments(min_hours=7000)` → **100 apartments**.
- **클라이언트 분리 없음**. 100가구 모두 학습 + 평가에 참여.
...
- 가구당 대략적 윈도우 수 (≈7000 시간 기준):
  ...
  → train pool ≈ 19,900 windows, val pool ≈ 2,400, test pool ≈ 5,300.
...
- **batch_size = 512** (conference Phase A 와 bit-equivalent — v04
  09_fix_rerun 도 동일).
- **rounds = 20, local_epochs = 2** (conference Phase A 와 동일).
- 풀 참여 (`C = 1.0`, 100/100 가구 매 라운드).
- Seed `{42, 123, 7}`.

> **B=512 의 의미.** 가구당 ≈ 199 train windows / B=512 → 가구당 1 mini-batch /
> epoch. 즉 local_epochs=2 → 가구당 라운드당 2 SGD step. 20 라운드 × 2 step
> = 40 SGD step / 가구. conference 가 이 setting 에서 cold PAPE 50–57 % 군집을
> 만들었으므로, v06 도 같은 setting 에서 in-train val trajectory 가 이
> 군집을 어떻게 형성하는지 보는 것이 목적. (`docs/fl_methodologies_fedsgd_vs_
> fedavg.md` §6 의 "B=64 sweep" 노트는 v06 본 실험 외의 v07+ 변형으로 보류.)
...
- 가구별 val PAPE/HR/MAE/MSE(kW²) 는 가구 내부 ≈ 24 윈도우 위에서 계산
  (z-space → kW 역정규화 후), 100 가구 평균이 `*_mean`, across-client
...
**Wall-clock 추산** (5070 Ti, NBEATSxAux ≈ 65 K params, 100가구, B=512):
- 라운드당 step ≈ 100 클라이언트 × 2 step × ~5 ms ≈ 1 s/round 의 compute +
  ~0.5 s aggregation + ~2 s val eval (100 클라이언트 × 24 윈도우 forward).
- V6-Dyn-B-{FedAvg, FedProx, FedRep, Ditto, FedProto}: 20 라운드 × ~3.5 s
  ≈ ~1 분 / cell / seed.
- V6-Dyn-A centralised: 40 epoch × 100 가구 × 1 batch × ~5 ms + 40 × 2 s
  eval ≈ ~1.5 분 / seed.
- Phase 1: 18 runs × ~1.2 분 ≈ ~22 분 직렬.
...
|---|---|---|---|
| **1** | `src/dataloader/per_client_split.py` (new) | `build_per_client_splits(seed) -> dict[apt -> {train_idx, val_idx, test_idx, mean, std}]`. 100 가구 전부, internal 70/10/20 sliding-window split. train z-norm only. seed 마다 deterministic. `outputs/v06_round_dynamics/seed{S}/per_client_split.pkl` 캐시. | pytest: 100 apts, 비율 + 비중복 verify. |
| **2** | `src/fl/round_logger.py` (new) | `RoundLogger`: open `round_log.jsonl` append-only; `log_round(round_idx, model, server_state_pre, client_states, val_loaders, comm_stats, wall)` 호출 시 모델로 100 클라이언트 val 윈도우 forward → per-client PAPE/HR/MAE/MSE(kW²) → across-client mean+std → 한 줄 write. resumable. | pytest: 2 라운드 dummy, jsonl 파싱, resume 동작. |
...
| **4** | `src/fl/centralised_pooled.py` (new) | V6-Dyn-A driver helper. 100 가구 train 윈도우 합친 단일 DataLoader, NBEATSxAux 학습. epoch 단위로 `on_round_end` 호출하여 logger 가 round-equivalent 행 작성. | pytest: 2 epoch 동기 작동. |
...
| **7** | `experiments/v06_round_dynamics/02_fl_dynamics.py` | V6-Dyn-B-{FedAvg, FedProx, FedRep, Ditto, FedProto} 통합 드라이버. argparse: `--seed S --algorithm {fedavg,fedprox,fedrep,ditto,fedproto} --rounds 20 --local_epochs 2 --batch 512`. **codebook 호출 없음** — Phase 1 은 backbone trajectory 만. | smoke `--seed 42 --algorithm fedavg --rounds 2`. |
...
A. **B = 512 의 epoch 해상도 한계.** 가구당 ≈ 199 train win / B=512 → 가구당 1 mini-batch / epoch. local_epochs=2 → 가구당 라운드당 2 SGD step. 따라서 *라운드 내부* 의 SGD 동역학은 관찰되지 않음 (라운드 단위 점만 찍힘). 만약 라운드 내부 dynamics 가 필요하면 conference hyperparameter 를 깨고 B=64 로 가야 함 (그러면 conference 비교 invariant 도 깨짐). 본 plan 은 conference invariant 우선, 라운드-내부 dynamics 는 v07+ 로 보류.
...
D. **NF / FM cell skip 여부.** Conference 종점 비교를 v06 trajectory 옆에 두려면 같은 100가구 internal val/test 위에서 재측정 필요 — conference 의 cold 20가구 결과를 그대로 옮길 수 없음. wall-clock 부담이 크면 D / E cell 은 v06 에서 빼고 paper draft 에서만 conference 결과를 인용.
...
- **Conference invariant.** 백본 / 알고리즘 / hyperparameter / 비교군 5종
  은 conference 와 동일. 평가 (cold zero-shot → per-round across-client val)
  만 변경.
