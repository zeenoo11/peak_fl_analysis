...
| v06 (`v06_round_dynamics`) | **drafted** | Round-level FL training dynamics — per-client 70/10/20 split (no cold partition), 6 cells (centralised + 5 FL) × 3 seeds × 20 rounds. Phase 1 = round-level trajectory; Phase 2 = post-hoc federated codebook stacking. 8 figures F1–F8. |
| v07 (`v07_loss_budget_sweeps`) | **drafted** | Loss-weight sensitivity sweep on the v06 protocol — λ_aux ∈ {0, 0.05, 0.1, 0.2, 0.3} × hr_weight ∈ {0.05, 0.1, 0.5, 1.0} × 6 algos × 3 seeds. Centralised has interior optimum λ=0.1; all 5 FL cells have strict boundary optimum λ=0 ("peak-aux is FL-incompatible at any positive λ"). v07-B (round budget) and v07-C (round-trajectory codebook) deferred. |
...
Comparison axis = **FedSGD vs FedAvg** (McMahan 2017's original two-methodology framing); see `docs/fl_methodologies_fedsgd_vs_fedavg.md` for the analysis. Headline experiment is an `E ∈ {1, 2, 5, 10, 20}` sweep at fixed total budget T=80 epoch-equivalent, plus a FedSGD reference (1 SGD step / round, ≈240 rounds), plus a centralised pooled SGD upper bound. Outputs are *round-vs-val-PAPE*, *bytes-vs-val-PAPE*, and *drift-vs-round* trajectory figures. The W5 / Peak-VQ codebook is **out of scope** for v06 (it is a cold-side correction module; without a cold partition it has no role). Scaffolding lives under `plans/v06-01_round_dynamics.md`; `experiments/v06_*/`, `papers/v06_draft/`, `outputs/v06_*/` will appear when implementation starts.
